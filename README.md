...
(it might also ask whether you want to overwrite certificate in that slot, but that's okay, just answer `y`)
	...
	$ python3 manage.py import_pubkey --user-email vsza@silentsignal.hu --attested-by {att,sig}-attest.pem
	...
	...
	$ python3 manage.py import_pubkey test.pub
	...
	$ python3 manage.py sign_cert 2 --identity teszt --principal teszt@silentsignal.hu -O force-command=uname -O clear
	Password: 
	...
	$ python3 manage.py trusted_ca_list
...
assuming you already have `ykman` installed as described above for OpenPGP.
	...
	...
	$ python3 manage.py import_pubkey --user-email vsza@silentsignal.hu --attested-by att-attest.pem sig-attest.pem
